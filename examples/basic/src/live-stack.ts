...
/**
 * Runs the full live AgentIO stack with real Noir, live 0G KV, and real local AXL nodes.
 *
 * This example is intentionally opt-in because it generates a real Noir proof,
 * writes to 0G, and starts real Gensyn AXL node processes. It demonstrates the
 * full trust boundary: Alice proves an authorized action, persists state on 0G,
 * sends the proof over AXL, and Bob verifies it before trusting the message.
 */

loadEnvFile();
const options = readOptions();

logTitle('0xAgentio full live stack');
logStep('Checking full live stack configuration');
logDetail('0G namespace', options.namespace);
logDetail('0G stream', options.streamId);
logDetail('AXL binary', options.axlBinaryPath);

const directory = await mkdtemp(join(tmpdir(), 'agentio-live-stack-'));
const aliceApiPort = Number(process.env.AGENTIO_AXL_ALICE_API_PORT ?? 19111);
const aliceListenPort = Number(
  process.env.AGENTIO_AXL_ALICE_LISTEN_PORT ?? 19211,
);
...
try {
  ...
  logStep(
    'Creating policy, credential, proof, storage, and transport adapters',
  );
  const policy = createPolicy({
    id: 'policy-live-stack-swap',
    allowedActions: ['swap'],
    constraints: [
  ...
  const credential = await issueLocalCredential({
    ...
    policy,
    id: `credential-live-stack-${Date.now()}`,
    issuedAt: now,
    signer: localDelegationSigner('principal-live-stack-demo'),
  });
  ...
  const action = createActionIntent({
    ...
    metadata: {
      assetPair: 'ETH/USDC',
      reason: 'full live stack smoke action',
    },
  });
  ...
  const alice = createAgentRuntime({
    ...
    execution: localVerifyingExecution(proof, async ({ action, proof }) => ({
      success: true,
      reference: `live-stack-local-execution:${proof.publicInputs.policyHash}:${action.type}`,
      details: {
    ...
    now: () => now,
    createEventId: () => `event-live-stack-${Date.now()}`,
  });
  ...
  bob.onMessage(async (message) => {
    if (message.type !== 'live-stack.action-proof') {
      return;
    }
  ...
  logStep('Sending Alice real Noir proof to Bob over real AXL transport');
  await alice.send(
    bob.identity.id,
    createAgentMessage({
      id: 'live-stack-action-proof-1',
      type: 'live-stack.action-proof',
  ...
  logStep('Full live stack complete');
...
function readOptions(): LiveStackOptions {
  const env = {
  ...
  const { evmRpc, indexerRpc, kvRpc, privateKey, streamId, axlBinaryPath } =
    env;
  if (
    evmRpc === undefined ||
    indexerRpc === undefined ||
    kvRpc === undefined ||
    axlBinaryPath === undefined
  ) {
  ...
  if (!isBytes32(streamId)) {
    throw new Error(
      'AGENTIO_0G_STREAM_ID must be a 0x-prefixed 32-byte stream id.',
    );
  }
  ...
  return {
    ...
    axlBinaryPath,
    namespace:
      process.env.AGENTIO_0G_NAMESPACE ?? `agentio-live-stack-${Date.now()}`,
    expectedReplica: readPositiveInteger(
      process.env.AGENTIO_0G_EXPECTED_REPLICA,
      2,
    ),
    logSyncTimeoutMs: readPositiveInteger(
      process.env.AGENTIO_0G_LOG_SYNC_TIMEOUT_MS,
      30_000,
    ),
    readRetryTimeoutMs: readPositiveInteger(
      process.env.AGENTIO_0G_KV_READ_RETRY_TIMEOUT_MS,
      120_000,
    ),
    readRetryIntervalMs: readPositiveInteger(
...
async function waitForAgentState(
  ...
  throw new Error(
    `${timeoutMessage}${stateReadbackDetail(lastState, lastError)}`,
  );
}
